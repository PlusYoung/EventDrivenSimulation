...
void SortedListClass::insertValue(
    ...
    // Condition 2: valToInsert is less than the head node value
        ...
        tempNode = new LinkedNodeClass(NULL, valToInsert, head);
        tempNode->setBeforeAndAfterPointers();
        head = tempNode;
    }
    // Condition 3: valToInsert is greater than or equal to the tail node value
    else if (tail->getValue() <= valToInsert) {
        tempNode = new LinkedNodeClass(tail, valToInsert, NULL);
        tempNode->setBeforeAndAfterPointers();
        tail = tempNode;
    }
    // Condition 4: valToInsert should be inserted into the middle
        ...
        while (!findLocation) {
            if (
                ...
                findLocation = true;
                LinkedNodeClass *inNode; // Represents the node to be inserted
                inNode = new LinkedNodeClass(tempNode, valToInsert,
                                             tempNode->getNext());
